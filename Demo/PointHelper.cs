...
namespace Demo {
    public class PointHelper {
        public static List<int []> GetPosition (Collider2D collider2D)
        {
            ...
            case CircleCollider2D polygonCollider2D:
                return GetPoints (polygonCollider2D);
            default:
                return new List<int []> ();
            }
        ...
        public static List<int []> GetPoints (CircleCollider2D circleCollider2D)
        {
            float radius = circleCollider2D.radius / 2f;
            Vector2 topLeftPoint = new (-radius, -radius);
            Vector2 bottomRightPoint = new (radius, radius);
            int [] worldTopLeft = ToWorldPoint (circleCollider2D, topLeftPoint);
            int [] worldBottomRight = ToWorldPoint (circleCollider2D, bottomRightPoint);
            return new List<int []> { worldTopLeft, worldBottomRight };
        }
